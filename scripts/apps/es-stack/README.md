这是为你整理的完整版 `README.md`。

你可以直接将以下内容保存为项目根目录下的 `README.md` 文件。它涵盖了项目介绍、目录结构、一键部署命令、Linux 系统设置提醒以及完整的测试流程。

---

```markdown
# Elasticsearch + Kibana + IK Analyzer 一键部署

这是一个基于 Docker Compose 和 Makefile 的 Elasticsearch (ES) + Kibana 快速部署方案，**已集成 IK 中文分词器**。

**主要特性：**
* 🚀 **一键启动**：自动处理插件下载、解压和挂载。
* 🇨🇳 **中文支持**：预装 `ik_smart` 和 `ik_max_word` 分词器，Kibana 已设为中文界面。
* ⚡ **网络优化**：利用宿主机网络下载插件，避免 Docker 内部构建时的网络超时问题。
* 🔓 **开发友好**：默认关闭 SSL 和 X-Pack 安全验证（无账号密码），方便本地开发调试。

---

## 📂 目录结构

```text
.
├── Makefile             # [核心] 自动化脚本：下载插件、管理容器生命周期
├── docker-compose.yml   # 容器编排配置
├── README.md            # 说明文档
└── plugins/             # [自动生成] 存放下载并解压后的 IK 分词器
└── es_data/             # 数据目录
```

---

## 🛠 前置要求

* **Docker** & **Docker Compose**
* **Make** (大多数 Linux/Mac 自带，Windows 用户需安装或使用 Git Bash)
* **unzip** (用于解压插件)
* **curl** (用于测试)

---

## 🚀 快速开始

### 1. 首次启动 (One-Click)

在项目根目录下执行：

```bash
make

```

> **脚本执行流程：**
>
> 1. 检查 `plugins/analysis-ik` 是否存在。
> 2. 不存在则自动从 Github 下载对应版本的 IK 插件 zip 包。
> 3. 解压插件到 `plugins/` 目录。
> 4. 启动 Docker Compose 服务。

### 2. 访问服务

启动成功后（需等待约 1 分钟）：

* **Elasticsearch**: [http://localhost:9200](https://www.google.com/search?q=http://localhost:9200)
* **Kibana**: [http://localhost:5601](https://www.google.com/search?q=http://localhost:5601)

---

## ⚠️ Linux 用户必读

Elasticsearch 对虚拟内存有要求。如果在 Linux 上启动失败（Exit Code 78），请执行：

```bash
# 临时生效
sudo sysctl -w vm.max_map_count=262144

# 永久生效 (写入配置文件)
echo "vm.max_map_count=262144" | sudo tee -a /etc/sysctl.conf
sudo sysctl -p

```

*(Mac 和 Windows Docker Desktop 用户通常无需此操作)*

---

## 🧪 验证与测试 (Curl)

你可以直接在终端复制以下命令，验证集群是否正常工作。

### 1. 检查节点与插件

```bash
# 查看 ES 版本 (应为 8.11.1)
curl -s "localhost:9200/?pretty" | grep number

# 查看已加载插件 (必须包含 analysis-ik)
curl -s "localhost:9200/_cat/plugins?v"

```

### 2. 测试中文分词

验证 IK 分词器是否生效。

**测试 `ik_smart` (粗粒度):**

```bash
curl -X POST "localhost:9200/_analyze?pretty" -H 'Content-Type: application/json' -d'
{
  "analyzer": "ik_smart",
  "text": "今天天气真不错"
}'

```

**测试 `ik_max_word` (细粒度):**

```bash
curl -X POST "localhost:9200/_analyze?pretty" -H 'Content-Type: application/json' -d'
{
  "analyzer": "ik_max_word",
  "text": "中华人民共和国国歌"
}'

```

### 3. 完整业务流程测试

模拟建索引、插入数据和搜索。

```bash
# 1. 创建索引 'news'，指定 title 字段使用 ik_max_word 分词
curl -X PUT "localhost:9200/news" -H 'Content-Type: application/json' -d'
{
  "mappings": {
    "properties": {
      "title": { "type": "text", "analyzer": "ik_max_word" }
    }
  }
}'

# 2. 插入一条文档
curl -X POST "localhost:9200/news/_doc/1" -H 'Content-Type: application/json' -d'
{ "title": "华为发布新款Mate60手机，遥遥领先" }'

# 3. 搜索关键词 "遥遥领先"
curl -X GET "localhost:9200/news/_search?pretty" -H 'Content-Type: application/json' -d'
{
  "query": { "match": { "title": "遥遥领先" } }
}'

```

---

## ⚙️ 运维命令 (Makefile)

| 命令           | 作用                                                                                                      |
| -------------- | --------------------------------------------------------------------------------------------------------- |
| `make`       | **推荐**。下载插件、解压并启动服务。                                                                |
| `make up`    | 仅执行 `docker compose up -d` (假设插件已准备好)。                                                      |
| `make down`  | 停止并移除容器 (数据保留在 Docker Volume 中)。                                                            |
| `make reset` | **危险**。停止容器，**删除插件目录**，**删除所有临时文件**。恢复到刚 Clone 下来的状态。 |

---

## 📝 版本信息

* **Elasticsearch**: 8.11.1
* **Kibana**: 8.11.1
* **IK Analyzer**: 8.11.1

# There are some TODOs for the project, and these shouldn't be edited by agents.

## v1.0.0

- [ ] 根据服务类型真实动态的检测服务运行与安装信息(p2)
- [ ] 网络连接根据协议实现详细的链路追踪(p1)
- [ ] 对于scripts中的app应该将其中的makefile提取在外部传入环境变量控制启动哪个app(p2)
- [ ] cmd目录下不应该包含具体的业务逻辑,应该将具体的业务逻辑放在app目录下,cmd目录下只应该包含具体的业务逻辑的调用(p0)
- [ ] history中应该加入迭代字段,记录操作的迭代实现溯源(p0)
- [ ] 文本压缩设计?根据调研判断可行性,以及压缩收益(p3)
- [ ] cli的具体用途与使用场景确定(p1)
- [ ] cli的命令行交互设计,类似于doctor其他的标准输出也应该拥有基本的表格或其他可视化样式(p1)
