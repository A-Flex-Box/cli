...
func NewCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "cli prompt 'add login API' -f go",
		Args:    cobra.MinimumNArgs(1),
		Run: func(cmd *cobra.Command, args []string) {
			s := GenerateTaskPrompt(pkgs.DefaultHistoryPath, strings.Join(args, " "), outputFormat)
			fmt.Println(s)
		},
	}
...
func newCommitCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			s, err := GenerateCommitPrompt(pkgs.DefaultHistoryPath, instruction)
			if err != nil {
				fmt.Printf("Error: %v\n", err)
